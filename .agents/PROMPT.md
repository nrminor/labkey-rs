...
1. Find the first story in `.agents/prd.json` where `passes: false`. Load the `jj` skill and then declare your intent for this commit with `jj describe` in the style discussed in @AGENTS.md. Then, proceed to working only on that story. Stories are ordered by dependency, so always work on the first incomplete one. Implementation should follow all guidelines from @AGENTS.md.
...
If, while implementing a feature, you notice the whole PRD is complete, output <promise>COMPLETE</promise>. This means you must never output that key unless the PRD is actually complete, as it will interrupt the agent loop.
